...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="main">
        ...
        <xsl:for-each select="collection(concat($recs,'?match=record-',if (normalize-space($nr)!='') then ($nr) else ('\d+'),'\.xml&amp;on-error=warning'))">
            ...
            <xsl:text expand-text="yes">{replace($rec//*:Header/*:MdSelfLink,'unl://','')}{$DELIM}{normalize-space(clariah:esc($rec//*:DataEnvelopeMetadata/(*:title[@xml:lang='en'],*:title)[1]))}</xsl:text>
