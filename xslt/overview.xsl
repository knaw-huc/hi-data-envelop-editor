...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="cwd" select="'file:/Users/menzowi/Documents/GitHub/hi-data-envelop-editor'"/>
    <xsl:param name="app" select="'data-envelopes'"/>
    <xsl:param name="config" select="doc(concat($cwd, '/data/apps/', $app, '/config.xml'))"/>
    
    <xsl:param name="tweakFile" select="'file:/Users/menzowi/Documents/GitHub/hi-data-envelop-editor/data/apps/data-envelopes/profiles/clarin.eu:cr1:p_1708423613607/clarin.eu:cr1:p_1708423613607.xml'"/>
    ...
    <xsl:function name="clariah:esc">
        <xsl:param name="val"/>
        <xsl:sequence select="if (matches($val,concat('^.*[',$STR,$DELIM,'].*$'))) then (concat($STR,replace($val,$STR,concat('\\',$STR)),$STR)) else ($val)"/>
    </xsl:function>
    ...
    <xsl:template name="main">
        ...
        <!--<xsl:text expand-text="yes">record_nr,record_id</xsl:text>
        <xsl:apply-templates select="$tweak" mode="header"/>
        <xsl:text expand-text="yes">{$REC}</xsl:text>-->
        ...
        <xsl:for-each select="collection(concat($recs,'?match=record-',if (normalize-space($nr)!='') then ($nr) else ('\d+'),'\.xml&amp;on-error=warning'))">
            ...
            <xsl:text expand-text="yes">{replace($rec//*:Header/*:MdSelfLink,'unl://','')}{$DELIM}{normalize-space(clariah:esc($rec//*:BasicInformation/(*:title[@xml:lang='en'],*:title)[1]))}</xsl:text>
    ...
    <xsl:template match="Element" mode="header">
        <xsl:variable name="e" select="."/>
        <xsl:text expand-text="yes">{$DELIM}{clariah:esc(@name)}</xsl:text>
        <xsl:if test="$e/@Multilingual='true'">
            <xsl:text expand-text="yes">{$DELIM}{clariah:esc(concat(@name,'_langs'))}</xsl:text>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="Element" mode="row">
        ...
        <xsl:message expand-text="yes">DBG: [{$e/@name}][{$p}]</xsl:message>
        <xsl:variable name="insts" as="node()*">
            <xsl:evaluate xpath="$p" context-item="$rec" namespace-context="$NS"/>
        </xsl:variable>
        <xsl:text expand-text="yes">{$DELIM}{count($insts)}</xsl:text>
        <xsl:if test="$e/@Multilingual='true'">
            <xsl:text expand-text="yes">{$DELIM}{string-join(distinct-values($insts/@xml:lang),' ')}</xsl:text>
        </xsl:if>                
    </xsl:template>
