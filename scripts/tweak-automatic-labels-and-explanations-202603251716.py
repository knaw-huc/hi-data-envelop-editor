...
V2_TWEAK_PATH = "/data/apps/data-envelopes-v2/profiles/clarin.eu:cr1:p_1770289476181/tweaks/tweak-0.xml"
...
def extract_v1_all_cues(v1_tree, full_mapping):
    """Extract ALL cue:* attributes from v1 elements, mapped to v2 paths.
    Returns dict: v2_path → dict of {cue_localname: value}
    e.g. {"/DataEnvelope/Uses/domains": {"class": "skosType"}}
    """
    v2_cues = {}
    for elem in v1_tree.getroot().iter():
        if elem.tag not in ("Component", "Element"):
            continue
        # Collect all cue:* attributes on this element
        cues = {}
        ...
        v2_path = full_mapping.get(v1_path)
        if v2_path:
            v2_cues[v2_path] = cues

    # Merge EXTRA_CUES (inputWidth/inputHeight for v2-only fields)
    for path, (w, h) in EXTRA_CUES.items():
        if path not in v2_cues:
            v2_cues[path] = {}
        if "inputWidth" not in v2_cues[path] and w:
            v2_cues[path]["inputWidth"] = w
        if "inputHeight" not in v2_cues[path] and h:
            v2_cues[path]["inputHeight"] = h
...
def process_node(elem, counters, depth, is_top_level, v2_explanations, v2_cues):
    ...
    # --- Cues (all cue:* attributes) ---
    if v2_path in v2_cues:
        for cue_name, cue_val in v2_cues[v2_path].items():
            elem.set(f"{{{CUE_NS}}}{cue_name}", cue_val)
...
def main():
    ...
    v2_cues = extract_v1_all_cues(v1_tree, full_mapping)
